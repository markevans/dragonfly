...
describe Dragonfly::DataStorage::FileDataStore do
  ...
  describe "with a given root path" do
    ...
    describe "store" do
      ...
      describe "when the filename already exists" do
        ...
        describe "specifying the uid" do
          it "should allow for specifying the path to use" do
            @data_store.store(content, :path => 'hello/there/mate.png')
      ...
      describe "return value" do
        ...
        it "should return the filepath without the root of the stored file when a file name is not provided" do
          @data_store.store(content).should == "#{@file_pattern_prefix_without_root}file"
        end
        ...
        it "should return the filepath without the root of the stored file when a file name is provided" do
          ...
          @data_store.store(content).should == "#{@file_pattern_prefix_without_root}hello.you.png"
        end
    ...
    describe "retrieve" do
      ...
      it "should throw :not_found if the file path has ../ in it" do
        expect{
          @data_store.retrieve(new_content, 'jelly_beans/../are/good')
        }.to throw_symbol(:not_found, 'jelly_beans/../are/good')
      end

      it "should throw :not_found if the file path has .. but not ../ in it" do
    ...
    describe "destroying" do
      ...
      it "should not prune root_path directory when destroying file without directory prefix in path" do
        uid = @data_store.store(content, :path => 'mate.png')
        @data_store.destroy(uid)
        @data_store.root_path.should be_an_empty_directory
      end

      it "should throw :not_found if the data doesn't exist on destroy" do
        uid = @data_store.store(content)
        @data_store.destroy(uid)
        lambda{
          @data_store.destroy(uid)
